...
baseamigus = Baseamigus (host='localhost', user='root', password='2104', database='amigurumis')
...
# Tratamiento de imagen
    ...
    if nuevo_codigo:
        ...
        return jsonify({"mensaje": "Amigu agregado correctamente.", "codigo": nuevo_codigo}), 201
    else:
        return jsonify({"mensaje": "Error al agregar el Amigu."}), 500
...
def mostrar_producto(codigo):
    ...
    if producto:
        ...
        return "Amigu no encontrado", 404
...
def eliminar_producto(codigo):
    ...
    if producto: 
        ...
        if baseamigus.eliminar_amigu(codigo):
            return jsonify({"mensaje": "Amigu eliminado"}), 200
        else:
            return jsonify({"mensaje": "Error al eliminar el Amigu"}), 500
    else: 
        return jsonify({"mensaje": "Amigu no encontrado"}), 404
...
def modificar_producto(codigo):
    ...
    if baseamigus.modificar_amigus(codigo, nueva_descripcion, nueva_nombre_autor, nombre_imagen):
        ...
        return jsonify({"mensaje": "Amigu modificado"}), 200
    else:
